...
while t < TStop
    ...
    if Method == 'FD'
        ...
        dvx(Pty0in) = Fx(Pty0in) * dt / Mass2;
        dvx(Pty1in) = Fx(Pty1in) * dt / Mass1;

        Vx = Vx + dvx;
        dx(Pty0in) = Vx(Pty0in) * dt + Fx(Pty0in) * dt^2 / 2 / Mass2;
        dx(Pty1in) = Vx(Pty1in) * dt + Fx(Pty1in) * dt^2 / 2 / Mass1;

        dvy(Pty0in) = Fy(Pty0in) * dt/Mass2;
        dvy(Pty1in) = Fy(Pty1in) * dt/Mass1;

        Vy = Vy + dvy;

        dy(Pty0in) = Vy(Pty0in) * dt + Fy(Pty0in) * dt^2 / 2 / Mass2;
    ...
    elseif Method == 'VE'
        ...
        x(Pty0in) = -xpp(Pty0in) + 2 * xp(Pty0in) + dt^2 / Mass2 * Fx(Pty0in);
        x(Pty1in) = -xpp(Pty1in) + 2 * xp(Pty1in) + dt^2 / Mass1 * Fx(Pty1in);

        y(Pty0in) = -ypp(Pty0in) + 2 * yp(Pty0in) + dt^2 / Mass2 * Fy(Pty0in);
